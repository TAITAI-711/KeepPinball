...
#define	POS_Y_CAM		(700.0f)			// カメラの初期位置(Y座標)
...
#define APPROACH_LIMIT		20
#define LEAVE_LIMIT			-50
...
static int				g_CameraUpLimit = 0;
static int				g_CameraApproachLimit = 0;
...
void UpdateCamera(void)
{
	...
	if (GetKeyboardPress(DIK_UP) && g_CameraUpLimit <= UP_LIMIT)
	{// 視点移動「上」
		g_Camera.pos.y += VALUE_MOVE_CAMERA;
		g_CameraUpLimit++;
	}

	if (GetKeyboardPress(DIK_DOWN) && g_CameraUpLimit >= DOWN_LIMIT)
	{// 視点移動「下」
		g_Camera.pos.y -= VALUE_MOVE_CAMERA;
		g_CameraUpLimit--;
	}
	...
	if (GetKeyboardPress(DIK_Q) && g_CameraApproachLimit <= APPROACH_LIMIT)
	{// 近づく
		g_Camera.len -= VALUE_MOVE_CAMERA;
		g_Camera.pos.x = g_Camera.at.x - sinf(g_Camera.rot.y) * g_Camera.len;
		g_Camera.pos.z = g_Camera.at.z - cosf(g_Camera.rot.y) * g_Camera.len;
		g_CameraApproachLimit++;
	}

	if (GetKeyboardPress(DIK_E) && g_CameraApproachLimit >= LEAVE_LIMIT)
	{// 離れる
		g_Camera.len += VALUE_MOVE_CAMERA;
		g_Camera.pos.x = g_Camera.at.x - sinf(g_Camera.rot.y) * g_Camera.len;
		g_Camera.pos.z = g_Camera.at.z - cosf(g_Camera.rot.y) * g_Camera.len;
	...
	// カメラを初期に戻す
	if (GetKeyboardPress(DIK_C) || GetKeyboardPress(DIK_X))
	{
		UninitCamera();
		InitCamera();
	}
